...
const Loans = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const response = await fetch((process.env.REACT_APP_API_URL || 'http://localhost:5000/api') + '/loans', {
            method: 'POST',
            headers: {
  ...
  const fetchLoanStats = async () => {
    try {
        const token = localStorage.getItem('token');
        const response = await fetch((process.env.REACT_APP_API_URL || 'http://localhost:5000/api') + '/loans/stats', {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
